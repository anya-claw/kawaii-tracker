...
// ==================== Auth ====================
export const api = {
    login: (token: string) => request('/api/login', { method: 'POST', body: JSON.stringify({ token }) }),
    ...
    deleteTask: (name: string) => request(`/api/tasks/${encodeURIComponent(name)}`, { method: 'DELETE' }),
    ...
    deleteTodoGroup: (id: number) => request(`/api/todo/groups/${id}`, { method: 'DELETE' }),
    ...
    completeTodoItem: (id: number) => request(`/api/todo/items/${id}/complete`, { method: 'POST' }).then(r => r.data),

    uncompleteTodoItem: (id: number) => request(`/api/todo/items/${id}/uncomplete`, { method: 'POST' }),

    deleteTodoItem: (id: number) => request(`/api/todo/items/${id}`, { method: 'DELETE' }),
